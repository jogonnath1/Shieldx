...
class _AdminComplaintsScreenState extends ConsumerState<AdminComplaintsScreen> {
  ...
  Future<void> _confirmBulkDelete(
      BuildContext context, List<dynamic> currentList) async {
    ...
    return Positioned(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Checkbox(
              value: _selectedIds.length == allFilteredComplaints.length &&
                  allFilteredComplaints.isNotEmpty,
            ...
            ElevatedButton.icon(
              onPressed: () =>
                  _confirmBulkDelete(context, allFilteredComplaints),
              style: ElevatedButton.styleFrom(
                backgroundColor: AppColors.error,
                foregroundColor: Colors.white,
                shape: RoundedRectangleBorder(
          ...
          .animate()
          .slideY(begin: 0.5, curve: Curves.easeOutBack, duration: 300.ms)
          .fadeIn(),
    );
  }
            ...
            Column(
              children: [
                    ...
                    children: [
                      ...
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            color: (filterState.category != 'all' ||
                                    filterState.date != null ||
                                    filterState.userVerification != 'all')
                ...
                if (complaintsAsync.hasValue &&
                    complaintsAsync.value!.isNotEmpty)
                  Padding(
                    padding:
                        const EdgeInsets.symmetric(horizontal: 16, vertical: 4),
                    child: Row(
                        ...
                        Checkbox(
                          ...
                          onChanged: (val) {
                            setState(() {
                              if (val == true) {
                                _selectedIds.addAll(
                                    complaintsAsync.value!.map((c) => c.id));
                              } else {
                                _selectedIds.clear();
                              }
                            });
    ...
    builder: (ctx) {
      return Consumer(
        builder: (ctx, ref, child) {
          final filterState = ref.watch(isAdmin
              ? adminComplaintFilterProvider
              : userComplaintFilterProvider);
          final filterNotifier = ref.read((isAdmin
                  ? adminComplaintFilterProvider
                  : userComplaintFilterProvider)
              .notifier);
          return SafeArea(
                          ...
                          children: [
                            ...
                            Expanded(
                              child: _VerificationSegmentButton(
                                ...
                                isSelected: filterState.userVerification ==
                                    'unverified',
                                onTap: () {
                                  filterNotifier.state = filterState.copyWith(
