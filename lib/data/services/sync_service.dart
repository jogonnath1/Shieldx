...
class SyncService {
  ...
  // --- Caching User Complaints ---
  Future<void> cacheUserComplaints(String userId, List<ComplaintModel> complaints) async {
  ...
  Future<void> addToOutbox(Map<String, dynamic> complaintData) async {
    ...
    debugPrint('SYNC: Added complaint ${complaintData['id']} to offline outbox. Total: ${outbox.length}');
    
    // Also prepend this offline complaint to the local user complaints cache
    ...
    if (userId != null) {
      ...
      final fullData = {
        ...
        'status': 'offline_pending', // Custom status indicator for offline complaints
      };
  ...
  Future<void> syncOfflineOutbox() async {
    ...
    debugPrint('SYNC: Starting background sync of ${outbox.length} offline complaints...');
    ...
    for (final itemStr in outbox) {
      try {
        ...
        // Strip custom local indicators if necessary, but keep it robust
        final dataToSubmit = Map<String, dynamic>.from(data);
        if (dataToSubmit['status'] == 'offline_pending') {
          dataToSubmit.remove('status'); // Let the server assign default 'submitted'
        }
        await complaintService.submitComplaint(dataToSubmit);
        debugPrint('SYNC: Successfully synced offline complaint ${data['id']} to Supabase!');
      } catch (e) {
    ...
    debugPrint('SYNC: Sync completed. Remaining in outbox: ${remaining.length}');
  }

  // --- Helper ---
  List<ComplaintModel> filterComplaintsByThana(List<ComplaintModel> list, String? stationThana) {
