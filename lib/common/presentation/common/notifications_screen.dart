...
class _NotificationsScreenState extends ConsumerState<NotificationsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final isAdmin =
        ref.watch(authNotifierProvider).valueOrNull?.isAdmin ?? false;
    return Scaffold(
      ...
      children: [
        if (todayItems.isNotEmpty) ...[
          ...
          ...todayItems
              .asMap()
              .entries
              .map((e) => _buildCard(e.value, isAdmin, e.key)),
        ],
        if (earlierItems.isNotEmpty) ...[
          _sectionHeader('Earlier'),
          ...earlierItems.asMap().entries.map(
              (e) => _buildCard(e.value, isAdmin, e.key + todayItems.length)),
        ],
      ],
    );
  }
  ...
  Future<void> _confirmDeleteSelected() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Are you sure you want to delete the ${_selectedIds.length} selected notifications? This action cannot be undone.',
          style: GoogleFonts.inter(
            color: AppColors.textSecondary,
            fontSize: 14,
            height: 1.5,
    ...
    if (confirmed == true) {
      ...
      if (mounted) {
        AppSnackbar.success(
            context, '${ids.length} notifications deleted successfully');
      }
    }
  }
  ...
  Future<void> _showUserProfileFromSos(NotificationModel n) async {
    ...
    try {
      ...
      if (mounted) {
        Navigator.pop(context);
        AppSnackbar.error(context, 'Error loading user profile: $e');
      }
    }
  }
}
