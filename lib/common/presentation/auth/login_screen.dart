  ...
  Future<void> _login() async {
    ...
    try {
      ...
      final isIncomplete = phone == null ||
          ...
          nid == null ||
          nid.trim().isEmpty;
      final destination = isIncomplete
          ? '/register'
          : (profile?.isAdmin == true ? '/admin/dashboard' : '/home');
      if (context.mounted) context.go(destination);
    } catch (e) {
      if (!mounted) return;
      final msg = e.toString().toLowerCase();
      if (msg.contains('blocked_by_admin') ||
          msg.contains('blocked') ||
          msg.contains('suspended')) {
        setState(() {
          _blockedError =
              'Your account has been suspended by the ShieldX Security Administrator due to violations of platform guidelines or security anomalies.';
        });
                ...
                Row(
                  children: [
                    TextButton.icon(
                      ...
                      onPressed: () {
                        Clipboard.setData(
                            const ClipboardData(text: 'support@shieldx.gov'));
                        AppSnackbar.success(
                            context, 'Support email copied to clipboard!');
                      },
