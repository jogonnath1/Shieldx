...
class EmergencyService {
  ...
  Future<void> _notifyAdmins({
    ...
    try {
      ...
      debugPrint('[EmergencyService] Failed to notify admins: $e');
    }
  }
  ...
  Future<EmergencyModel> triggerSOS(double lat, double lng) async {
    ...
    await _notifyAdmins(
      ...
      message:
          '$citizenName ($citizenPhone) has triggered an active SOS alert! Coordinates: $lat, $lng.',
      type: 'sos',
  ...
  Future<void> cancelEmergency(String emergencyId) async {
      ...
      if (emergencyRes != null) {
        ...
        if (userId != null) {
          final profileRes = await _client
              ...
              .eq('id', userId)
              .maybeSingle();
          final name = profileRes != null
              ? (profileRes['name'] ?? 'A citizen')
              : 'A citizen';
          final phone =
              profileRes != null ? (profileRes['phone'] ?? 'N/A') : 'N/A';
          try {
            await _client
                .from('notifications')
                .delete()
            ...
            debugPrint(
                '[EmergencyService] Failed to delete SOS notifications: $e');
          }
          await _notifyAdmins(
            title: '✅ SOS Cancelled / Safe',
            message:
                '$name ($phone) has marked themselves safe and cancelled the SOS alarm.',
            type: 'sos',
    ...
    await _client.from('emergencies').update({
      'status': 'cancelled',
  ...
  Stream<List<EmergencyModel>> watchActiveEmergencies() {
    return _client
        ...
        .asyncMap((data) async {
          ...
          final activeRows =
              data.where((row) => row['status'] == 'active').toList();
          ...
          for (final row in uniqueActiveRows) {
            try {
              final profileRes = await _client
                  ...
                  .maybeSingle();
              final profile =
                  profileRes != null ? ProfileModel.fromMap(profileRes) : null;
