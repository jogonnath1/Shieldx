...
class ProfileService {
  ...
  Future<void> setVerified(String userId, bool verified) async {
    await _client
        .from(AppConstants.profilesTable)
        .update({'is_verified': verified}).eq('id', userId);
    try {
      const uuid = Uuid();
      await _client.from('notifications').insert({
        ...
        'user_id': userId,
        'title':
            verified ? 'Account Verified ✅' : 'Verification Status Updated',
        'message': verified
      ...
      debugPrint('Failed to send verification notification: $e');
    }
  }
  ...
  Future<void> setBlocked(String userId, bool blocked) async {
    ...
    await _client
        .from(AppConstants.profilesTable)
        .update({'is_blocked': blocked}).eq('id', userId);
  }
}
