...
class EditComplaintScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<EditComplaintScreen> createState() =>
...
class _EditComplaintScreenState extends ConsumerState<EditComplaintScreen> {
  ...
  Future<void> _loadComplaint() async {
    ...
    if (c == null || c.status != 'submitted') {
      if (mounted) {
        AppSnackbar.info(context, 'This report can no longer be edited.');
        if (context.canPop()) {
          context.pop();
        } else {
  ...
  Future<void> _save() async {
    ...
    if (_incidentDate == null) {
      AppSnackbar.error(context, 'Please select incident date & time');
      return;
    }
    if (_incidentDate!.isAfter(DateTime.now().toBangladeshTime())) {
      AppSnackbar.error(
          context, 'Incident date & time cannot be in the future.');
      return;
    }
    ...
    if (picked != null) {
      ...
      final time = await showTimePicker(
        ...
        initialTime: TimeOfDay.fromDateTime(
            _incidentDate ?? DateTime.now().toBangladeshTime()),
        builder: (ctx, child) => Theme(
          data: Theme.of(ctx).copyWith(
            colorScheme: const ColorScheme.dark(primary: AppColors.primary),
          ),
        ...
        if (selectedDateTime.isAfter(now)) {
          ...
          if (mounted) {
            AppSnackbar.error(context,
                'Future time is not allowed. Reverted to current time.');
            return;
          }
          return;
        }
