...
The culmination of this project represents not only our dedication but also the exceptional academic guidance and visionary mentorship that shaped it.\\[0.5cm]
...
\begin{tikzpicture}[
    ...
    \draw [dashedarrow] (uc_filter) -- node[above, sloped, font=\footnotesize] {$\ll$extend$\gg$} (ua_manage);
...
\begin{tikzpicture}[scale=0.85, transform shape,
    ...
    entity/.style={rectangle, draw=black, thick, fill=blue!5, text width=4cm, align=center, rounded corners, inner sep=5pt},
    relation/.style={-{Stealth[scale=1.2]}, thick},
    lbl/.style={font=\scriptsize, align=center, fill=white, inner sep=2pt}
]
% Center Entities
\node[entity] (prof) at (0, 4) {\textbf{profiles} \\ \hrule\vspace{2pt} id (PK) \\ role \\ is\_verified \\ fcm\_token};
\node[entity] (comp) at (0, -2) {\textbf{complaints} \\ \hrule\vspace{2pt} id (PK) \\ user\_id (FK) \\ assigned\_officer\_id (FK) \\ status};
% Top Orbit
\node[entity] (emerg) at (-5, 8) {\textbf{emergencies} \\ \hrule\vspace{2pt} id (PK) \\ user\_id (FK) \\ resolved\_by (FK)};
\node[entity] (notif) at (5, 8) {\textbf{notifications} \\ \hrule\vspace{2pt} id (PK) \\ user\_id (FK) \\ related\_id};
% Middle Orbit
\node[entity] (msg) at (-6, -2) {\textbf{messages} \\ \hrule\vspace{2pt} id (PK) \\ complaint\_id (FK) \\ sender\_id (FK)};
\node[entity] (off) at (6, -2) {\textbf{officers} \\ \hrule\vspace{2pt} id (PK) \\ user\_id (FK) \\ station};
% Bottom Orbit
\node[entity] (hist) at (0, -8) {\textbf{status\_history} \\ \hrule\vspace{2pt} id (PK) \\ complaint\_id (FK) \\ changed\_by (FK)};
% Primary Relationships from Profiles
\draw[relation] (prof.north west) -- node[above, lbl, sloped] {user\_id (1:N)} (emerg.south east);
\draw[relation] (prof.north east) -- node[above, lbl, sloped] {user\_id (1:N)} (notif.south west);
\draw[relation] (prof.south) -- node[left, lbl] {user\_id (1:N)} (comp.north);
% Primary Relationships from Complaints
\draw[relation] (comp.west) -- node[above, lbl] {complaint\_id (1:N)} (msg.east);
\draw[relation] (comp.east) -- node[above, lbl] {assigned\_officer\_id (N:1)} (off.west);
\draw[relation] (comp.south) -- node[left, lbl] {complaint\_id (1:N)} (hist.north);
% Secondary Relationships (Dashed to avoid clutter)
\draw[relation, dashed] (prof.west) -| node[above, lbl, near start] {sender\_id (1:N)} (msg.north);
\draw[relation, dashed] (prof.east) -| node[above, lbl, near start] {user\_id (1:1)} (off.north);
% Complex Routing around officers to reach status_history
\draw[relation, dashed] (prof.20) -- ++(6.5, 0) |- node[right, lbl, near start] {changed\_by (1:N)} (hist.340);
% Complex Routing to reach emergencies resolved_by without crossing anything
\draw[relation, dashed] (prof.160) -| node[left, lbl, near start] {resolved\_by (1:N)} (emerg.280);
\end{tikzpicture}
...
\begin{tikzpicture}[scale=0.9, transform shape,
    every node/.style={font=\sffamily},
    process/.style={circle, draw=black, thick, minimum size=3cm, align=center, fill=blue!5},
    entity/.style={rectangle, draw=black, thick, minimum width=3cm, minimum height=1.5cm, fill=orange!5},
    arrow/.style={-{Stealth[scale=1.2]}, thick},
    lbl/.style={font=\scriptsize, align=center, fill=white, inner sep=2pt}
]
% Level 0 Nodes
\node[process] (system) at (0,0) {0.0 \\ ShieldX \\ System};
\node[entity] (citizen) at (-6, 0) {Citizen};
\node[entity] (admin) at (6, 2) {Police Admin};
\node[entity] (gps) at (6, -2) {GPS \& Map Services};
% Citizen -> System
\draw[arrow] (citizen.40) -- node[above, lbl, sloped] {Complaints, SOS, Profile Data} (system.140);
% System -> Citizen
\draw[arrow] (system.220) -- node[below, lbl, sloped] {Complaint Status, App UI} (citizen.320);
% Admin -> System
\draw[arrow] (admin.190) -- node[above, lbl, sloped] {Verifications, Updates} (system.40);
% System -> Admin
\draw[arrow] (system.10) -- node[below, lbl, sloped] {Analytics, Live SOS} (admin.230);
% System -> GPS
\draw[arrow] (system.350) -- node[above, lbl, sloped] {Coordinate Requests} (gps.130);
% GPS -> System
\draw[arrow] (gps.170) -- node[below, lbl, sloped] {Real-time Location} (system.310);
\end{tikzpicture}
