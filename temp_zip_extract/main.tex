...
\draw[arrow] (citizen.north) |- ++(0,1.5) -| node[above, lbl, near start] {SOS Trigger} (p3.north);
% Flows from Admin
\draw[arrow] (admin.west) -- node[above, lbl] {View Reports} (p4.east);
\draw[arrow] (admin.east) -| ++(1.5, 3) |- node[right, lbl, near start] {Verify Users} (p1.east);
\draw[arrow] (admin.south) |- ++(0,-1.5) -| node[below, lbl, near start] {Update Status} (p2.south);
